    ...
    public class FeedbackEngine
    {
        ...
        private void Load()
        {
            ...
            try
            {
                ...
                ComputeMultipliers();
                SimHub.Logging.Current.Info($"[K10Motorsports] Loaded {_entries.Count} feedback entries");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Warn($"[K10Motorsports] Could not load feedback: {ex.Message}");
            }
        }
        ...
        public void Record(string topicId, string promptText, int rating)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10Motorsports] Could not save feedback: {ex.Message}");
            }
