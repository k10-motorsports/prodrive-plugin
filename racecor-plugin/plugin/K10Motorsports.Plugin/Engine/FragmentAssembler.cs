...
namespace K10Motorsports.Plugin.Engine
{
    ...
    public class FragmentAssembler
    {
        ...
        public void LoadFragments(string jsonPath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[K10Motorsports.FragmentAssembler] Fragments file not found: {jsonPath}");
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[K10Motorsports.FragmentAssembler] Loaded {_fragments.Count} fragment topics from {jsonPath}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[K10Motorsports.FragmentAssembler] Failed to load fragments: {ex.Message}");
            }
        }
