# Apple HomeKit Light Control for RaceCor

This guide walks through connecting your Apple HomeKit-compatible smart lights to the RaceCorProDrive plugin, so your lights react to what's happening on track.

## Prerequisites

Before starting, you need the following running on your network:

- **SimHub** with the RaceCorProDrive plugin installed and active
...
You should see a `0` or `1` response. If you get an error, confirm the RaceCorProDrive plugin is loaded and SimHub's web server is active.

### 3. Configure in Homebridge

Open the Homebridge UI and find **RaceCorProDrive Lights** in the plugin list. Add a platform entry to your Homebridge `config.json`:
...
Create a file called `racecor-lights.js` somewhere accessible to Homebridge (e.g., alongside your `config.json`):

```javascript
// racecor-lights.js
// Mirrors RaceCorProDrive virtual light colors to physical Homebridge lights

// Configuration — change these to match your accessory names
const VIRTUAL_LIGHT = 'Sim Rig Light';       // Name of the RaceCorProDrive virtual light
const PHYSICAL_LIGHTS = ['Desk Lamp', 'LED Strip'];  // Names of your real lights

automation.listen(({ serviceName, characteristic, value }) => {
  // Only react to changes on the RaceCorProDrive virtual light
...
{
  "platform": "Automation",
  "automationCode": "/path/to/racecor-lights.js"
}
```
...
1. Start a race session in SimHub (or enable Demo mode in the RaceCorProDrive plugin settings)
...
- Check Homebridge logs for connection errors from the RaceCorProDrive Lights plugin.
- Ensure the RaceCorProDrive plugin is active in SimHub (look for "RaceCorProDrive.Plugin" in SimHub's plugin list).
