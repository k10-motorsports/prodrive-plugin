...
namespace MediaCoach.Plugin.Engine
{
    ...
    public class FragmentAssembler
    {
        ...
        public void LoadFragments(string jsonPath)
        {
            ...
            {
                SimHub.Logging.Current.Warn($"[MediaCoach.FragmentAssembler] Fragments file not found: {jsonPath}");
                return;
            }
            ...
            try
            {
                ...
                SimHub.Logging.Current.Info($"[MediaCoach.FragmentAssembler] Loaded {_fragments.Count} fragment topics from {jsonPath}");
            }
            catch (Exception ex)
            {
                SimHub.Logging.Current.Error($"[MediaCoach.FragmentAssembler] Failed to load fragments: {ex.Message}");
            }
        }
